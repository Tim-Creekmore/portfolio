Shader "Custom/Trunk"
{
    Properties
    {
        _BarkColor     ("Bark Color", Color) = (0.541, 0.376, 0.251, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float barkNoise = noise3(wp * 4.0);
                float barkGrain = noise3(float3(wp.x * 2.0, wp.y * 12.0, wp.z * 2.0 + 3.0));

                float3 col = lerp(_BarkColor.rgb, IN.vertColor, 0.6);
                col += (barkNoise - 0.5) * 0.08;
                col *= 0.92 + barkGrain * 0.12;

                float moss = smoothstep(0.52, 0.62, noise3(wp * 2.0 + float3(0, 0, 7)));
                col = lerp(col, float3(0.18, 0.28, 0.1), moss * 0.25);
                col = saturate(col);
