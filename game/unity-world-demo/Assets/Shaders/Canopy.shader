Shader "Custom/Canopy"
{
    Properties
    {
        _LeafColor    ("Leaf Color", Color) = (0.24, 0.42, 0.16, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 dark  = leafCol * 0.55;
                float3 light = leafCol * 1.4 + float3(0.06, 0.10, 0.0);
                float3 col = lerp(dark, leafCol, leafMask);
                col = lerp(col, light, leafEdge * 0.45);
                col += (n3 - 0.5) * 0.08;

                float upDot = dot(wn, float3(0, 1, 0)) * 0.5 + 0.5;
                float ssao = lerp(0.55, 1.0, upDot);
                col *= ssao;
                col = saturate(col);
